...
class ConfigManager:
    ...
    def __init__(self):
        ...
        self.default_config = {
            ...
            "mic_osd_style": "waveform",  # Visualization style: 'waveform', 'vu_meter', or 'pill'
    ...
    def _load_config(self):
        ...
        try:
            if self.config_file.exists():
                ...
                if (
                    "push_to_talk" in loaded_config
                    and "recording_mode" not in loaded_config
                ):
                ...
                if (
                    "audio_device" in loaded_config
                    and "audio_device_id" not in loaded_config
                ):
    ...
    def migrate_api_key_to_credential_manager(self) -> bool:
        ...
        if save_credential(identified_provider, api_key):
            ...
            self.config["rest_api_key"] = (
                None  # Set to None instead of deleting for backward compat
            )
            self.save_config()
            ...
            try:
                ...
                log_info(
                    f"Migrated API key to credential manager (provider: {identified_provider})",
                    "CONFIG",
                )
            except ImportError:
                print(
